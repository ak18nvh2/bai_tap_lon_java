...
public class SinhVien {
    ...
    public ResultSet ShowSinhVienYeuCauGiaHan() {
        try {
            cn.ConnectSQL();
            String sql = "Select * from sinhvien where YeuCauGiaHan='1'";
            return cn.LoadData(sql);
        } catch (SQLException ex) {
            ex.printStackTrace();
        }
     ...
     public ResultSet DemSinhVienYeuCauGiaHan() {
        try {
            cn.ConnectSQL();
            String sql = "SELECT COUNT(*) AS \"Tong\" FROM `sinhvien` WHERE YeuCauGiaHan>0";
            return cn.LoadData(sql);
        } catch (SQLException ex) {
            ex.printStackTrace();
        }
    ...
    public void InsertData(String maSV, String hoTen, int gioiTinh, String soCMT, String ngaySinh, String queQuan,
            String soDT, String email, String hocLuc, String loaiPhong, String taiKhoan, String matKhau, String hanhKiem, int dangKy, int thongBao) {
        try {
            cn.ConnectSQL();

            String sql = "INSERT INTO sinhvien values('" + maSV + "',N'" + hoTen + "','" + gioiTinh + "','" + soCMT + "','" + ngaySinh
                    + "','" + queQuan + "','" + soDT + "','" + email + "','" + hocLuc + "','"+hanhKiem+"', NULL, NULL, '" + loaiPhong + "',NULL, '" + taiKhoan + "','" + matKhau + "','"+dangKy+"','"+thongBao+"')";
            cn.UpdateData(sql);
        } catch (SQLException ex) {
            Logger.getLogger(SinhVien.class.getName()).log(Level.SEVERE, null, ex);
